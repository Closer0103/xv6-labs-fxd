...
void ukvmmap(pagetable_t kernel_pgtbl,uint64 va, uint64 pa, uint64 sz, int perm)//映射制定页表的项，类似于kvmap的辅助函数
{
...
pagetable_t ukvminit()//对指定页表进行映射，仿照已有的函数kvminit完成。
{
...
int kvmcopymappings(pagetable_t src, pagetable_t dst, uint64 start, uint64 sz)
{
  ...
  {
    if((pte = walk(src, i, 0)) == 0) /*寻找最后一级页表项*/
      panic("kvmcopymappings: pte should exist");
    if((*pte & PTE_V) == 0)	/*判断页表项的有效性*/
...
uint64 kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz))/*若存在多余的页，需要在这里进行释放*/
  {
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
